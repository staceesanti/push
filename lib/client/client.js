...
/*
  ...
  @param channels list of channels from the device
  @param callback function to be invoked if there were
  no errors thrown in raix:push-update
 */
Push.reportTokenToServer = function (token, appName, platform, channels, callback) {
  // Store the token
  stored.token = token;
  ...
  let data = {
    ...
    metadata: stored.metadata,
    channels: channels,
    buildVersion: cordova.Vet2PetBuildVariables.buildVersion
  };
  ...
  Meteor.startup(function () {
    ...
    Push.on('token', function (token) {
      ...
      if (/android/i.test(device.platform)) {
        PushNotification.listChannels(
          (result) => {
            Push.reportTokenToServer(token, appName || 'main', undefined, result);
          },
          () => {
