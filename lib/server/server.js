...
function getApp(query) {
  let app = GlobalGenericApps.findOne(query);
  if (!app) {
    app = GlobalVets.findOne(query);
  }
  return app;
}
...
function initializeFirebaseAppForVetId(vetId) {
  ...
  if (!app || !app.firebaseConfig || !app.firebaseConfig.adminSdkCredentials) {
...
function convertApnToFcmToken(appName, tokenObject) {
  ...
  const query = {appIdentifier: appName};
  let app = getApp(query);
