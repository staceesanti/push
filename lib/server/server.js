...
Push.firebaseManagement = {
  ...
  checkIfRequiredGoogleApisAreEnabled: (vetId) => {
    return checkIfRequiredGoogleApisAreEnabled(vetId);
  },
  uploadAndroidSHA256Fingerprint: (vetId, fingerprint) => {
    return uploadAndroidSHA256Fingerprint(vetId, fingerprint);
  }
};
...
Meteor.methods({
  ...
  'raix:configure-push-connections': (vetId) => {
    ...
    initializeFirebaseAppForVetId(vetId);
  },
...
function initializeFirebaseAppForVetId(vetId) {
  const vet = GlobalVets.findOne({
    _id: vetId,
  ...
  let key = vet.appIdentifier;
  let projectId = vet.firebaseConfig.adminSdkCredentials.projectId;
  let clientEmail = vet.firebaseConfig.adminSdkCredentials.clientEmail;
  let privateKey = vet.firebaseConfig.adminSdkCredentials.privateKey;
...
function convertApnToFcmToken(appName, tokenObject) {
  ...
  HTTP.post('https://iid.googleapis.com/iid/v1:batchImport', {
    ...
    data: {
      'application': GlobalAppUtils.getIosBundleId(vet),
      'sandbox': false,
...
function createApp(projectManagement, vet, platform) {
  switch (platform) {
    case PLATFORMS.ANDROID: {
      return projectManagement.createAndroidApp(GlobalAppUtils.getAndroidPackageName(vet));
    }
    case PLATFORMS.IOS: {
      return projectManagement.createIosApp(GlobalAppUtils.getIosBundleId(vet));
    }
    default: {
      throw new Error(`Invalid platform: ${platform}`);
    }
...
function checkIfRequiredGoogleApisAreEnabled(vetId) {
  return new Promise((resolve, reject) => {
    if (!vetId) {
      reject(new Error('Vet ID is required.'));
      return;
...
function uploadAndroidSHA256Fingerprint(vetId, fingerprint) {
  return new Promise((resolve, reject) => {
    if (!vetId) {
      reject(new Error('Vet ID is required.'));
      return;
    ...
    listApps(projectManagement, PLATFORMS.ANDROID).then(apps => {
      ...
      if (!apps || apps.length === 0) {
        reject(new Error(`Could't find android app for vet id ${vetId}`));
      } else if (apps.length === 1) {
