...
Meteor.methods({
  ...
  'raix:subscribe-to-topic': (appIdentifier, topicName, fcmToken) => {
    ...
    firebaseApp.messaging().subscribeToTopic([fcmToken], `/topics/${topicName}`).then((response) => {
  ...
  'raix:unsubscribe-from-topic': (appIdentifier, topicName, fcmToken) => {
    ...
    firebaseApp.messaging().unsubscribeFromTopic([fcmToken], `/topics/${topicName}`).then((response) => {
      // See the MessagingTopicManagementResponse reference documentation
      // for the contents of response.
