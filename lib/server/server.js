...
function subscribeToTopic(tokenId, appIdentifier, topicName) {
  let pushToken = Push.appTokens.findOne({_id: tokenId});
  ...
  firebaseApp.messaging().subscribeToTopic([pushToken.token.fcm], `/topics/${topicName}`).then((response) => {
