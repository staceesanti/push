...
Meteor.methods({
  ...
  'raix:subscribe-to-topic': (appIdentifier, topicName, fcmToken) => {
    check(appIdentifier, String);
    ...
    retryFunction(() => {
      ...
      firebaseApp.messaging().subscribeToTopic([fcmToken], `/topics/${topicName}`).then()
        .catch((error) => {
  ...
  'raix:unsubscribe-from-topic': (appIdentifier, topicName, fcmToken) => {
    check(appIdentifier, String);
    check(topicName, String);
    check(fcmToken, String);
    ...
    retryFunction(() => {
      ...
      firebaseApp.messaging().unsubscribeFromTopic([fcmToken], `/topics/${topicName}`).then()
        .catch((error) => {
...
function retryFunction(fn, numberOfTries) {
