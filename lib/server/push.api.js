...
Push.Configure = function (options, desktopApps) {
  const self = this;
  ...
  if (desktopApps && desktopApps.length) {
    desktopApps.forEach(appConfig => {
...
function querySend(query, raixNotification) {
  ...
  let desktopAppId = raixNotification && raixNotification.payload && raixNotification.payload.isDesktop && raixNotification.payload.vetId || undefined;
