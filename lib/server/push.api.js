...
Push.setBadgeFunction = function(func) {
  Push.getBadgeNumber = func;
};
...
function getFcmMessage(raixNotification, platform) {
  let badgeNumber = Push.getBadgeNumber(raixNotification);
  if (platform === 'Android') {
    return {
      data: {
        ...
        badge: badgeNumber ? badgeNumber.toString() : '1'
      }
    };
  }
    ...
    apns: {
      ...
      payload: {
        aps: {
          ...
          badge: badgeNumber || raixNotification.apn.badge || raixNotification.badge
        }
      }
    }
  };
