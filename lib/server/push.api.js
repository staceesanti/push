...
Push.Configure = function (options, desktopAppsFirebaseConfigurations) {
  ...
  apps.forEach((app) => {
    Meteor.call('raix:configure-push-connections', app.appIdentifier);
  });
...
function querySend(query, raixNotification) {
  ...
  let vetId = raixNotification.payload.vetId;

  let appTokens = AppTokenUtil.getAppTokensBasedOnQuery(query, desktopAppId, vetId);
...
function topicsSend(topics, raixNotification) {
  ...
  console.log('Push: Sending notification to topic(s): ', raixNotification);

  const appIdentifier = raixNotification.appIdentifier;
  topics.forEach(function (topic) {
    if (topic === 'android_users') {
      firebasePromises.push(sendFcmMessage(appIdentifier, getFcmMessageForTopic(topic, raixNotification, 'android')));
    } else if (topic === 'ios_users') {
      firebasePromises.push(sendFcmMessage(appIdentifier, getFcmMessageForTopic(topic, raixNotification, 'ios')));
    } else {
      firebasePromises.push(sendFcmMessage(appIdentifier, getFcmMessageForTopic(topic, raixNotification, 'android')));
      firebasePromises.push(sendFcmMessage(appIdentifier, getFcmMessageForTopic(topic, raixNotification, 'ios')));
    }
