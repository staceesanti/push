...
Push.legacy.Configure = function (options) {
  ...
  // Add debug info
  if (Push.debug) {
    console.log('Push (legacy): Push.legacy.Configure', options);
  }
  ...
  _replaceToken = function (currentToken, newToken) {
    // console.log('Push (legacy): Replace token: ' + currentToken + ' -- ' + newToken);
  ...
  _removeToken = function (token) {
    console.log('Push (legacy): Remove token: ' + token);
    // Invalidate the token
    Push.emitState('token', token, null);
  };

  if (Push.debug) {
    console.log('Push (legacy): APN configured');
  }

  const apn = Npm.require('apn');

  const useProductionCert = process.env.NODE_ENV === 'development';

  if (!useProductionCert) {
    console.log('Push (legacy): This is being configured in development mode');
  }
  ...
  const vets = GlobalVets.find({
    $and: [
      ...
      {genericAppId: {$exists: false}}
    ]
  }).fetch();

  const genericApps = GlobalGenericApps.find({
    disabled: {$ne: true},
    useFirebaseForMessaging: {$ne: true}
  }).fetch();

  const apps = [...vets, ...genericApps];
  apps.forEach((app) => {
    let daysRemaining = Meteor.call('checkCertificate', app._id);

    if (daysRemaining > 1) {
      Meteor.call('raix:configure-push-connections-legacy', app._id);
    }
  });
  ...
  Push.legacy._sendAPN = function (userToken, notification) {
    ...
    // console.log('Push (legacy): _sendAPN', notification.from, userToken, notification.title, notification.text,
    ...
    console.log('Push (legacy): Send message to: ' + userToken);
  ...
  Push.legacy._sendGCM = function (userTokens, notification) {
    ...
    if (!userTokens.length) {
      if (Push.debug) {
        console.log('Push (legacy): _sendGCM no push tokens found');
      }
      return;
    }

    if (Push.debug) {
      console.log('Push (legacy): _sendGCM', userTokens, notification);
    }
    ...
    if (Push.debug) {
      console.log('Push (legacy): Create GCM Sender using "' + Push.legacy.Connections[key].gcm.apiKey + '"');
    }
    const sender = new gcm.Sender(Push.legacy.Connections[key].gcm.apiKey);

    _.each(userTokens, function (value /* , key */) {
      if (Push.debug) {
        console.log('Push (legacy): A:Send message to: ' + value);
      }
    });
    ...
    sender.send(message, userTokens, 5, function (err, result) {
      if (err) {
        if (Push.debug) {
          console.log('Push (legacy): ANDROID ERROR: result of sender: ' + result);
        }
      } else {
        if (result === null) {
          if (Push.debug) {
            console.log('Push (legacy): ANDROID: Result of sender is null');
          }
          return;
        }
        if (Push.debug) {
          console.log('Push (legacy): ANDROID: Result of sender: ' + JSON.stringify(result));
        }
    ...
    //     console.log('Push (legacy): ANDROID: ' + JSON.stringify(result));
    // });
    // **/
  }; // EO sendAndroid
  ...
  const _querySend = function (query, options) {
    ...
    console.log('Push (legacy): Sending notifications for query: ', query);

    let appTokens = AppTokenUtil.getAppTokensBasedOnQuery(query);
    appTokens.forEach(function (app) {
      if (Push.debug) {
        console.log('Push (legacy): Iterating...', app);
        console.log('Push (legacy): send to token', app.token);
      }

      if (app.token.apn) {
        if (Push.debug) {
          console.log('Push (legacy): app.token.apn', 'passed');
        }
        countApn.push(app._id);
        // Send to APN
        if (Push.legacy._sendAPN) {
          if (Push.debug) {
            console.log('Push (legacy): sending apn');
          }
    ...
    if (Push.debug) {
      console.log('Push (legacy): Sent message "' + options.title + '" to ' + countApn.length + ' ios apps ' +
      ...
      if (!countApn.length && !countGcm.length) {
        if (Push.appTokens.find().count() === 0) {
          console.log('Push (legacy): Push, GUIDE: The "Push.appTokens" is empty -' +
            ' No clients have registred on the server yet...');
        }
      } else if (!countApn.length) {
        if (Push.appTokens.find({'token.apn': {$exists: true}}).count() === 0) {
          console.log('Push (legacy): Push, GUIDE: The "Push.appTokens" - No APN clients have registred on the server yet...');
        }
      } else if (!countGcm.length) {
        if (Push.appTokens.find({'token.gcm': {$exists: true}}).count() === 0) {
          console.log('Push (legacy): Push, GUIDE: The "Push.appTokens" - No GCM clients have registred on the server yet...');
        }
      }
  ...
  Push.legacy._serverSend = function (options) {
    ...
    if (options.title !== '' + options.title && options.text !== '' + options.text) {
    ...
    if (options.token || options.tokens) {
      ...
      if (Push.debug) {
        console.log('Push (legacy): Send message "' + options.title + '" via token(s)', tokenList);
      }
      ...
      if (Push.debug) {
        console.log('Push (legacy): Send message "' + options.title + '" via query', options.query);
      }
  ...
  if (cfgOptions.sendInterval !== null && !(process.env.PUSH_NOTIFICATIONS_DISABLED === true || process.env.PUSH_NOTIFICATIONS_DISABLED === 'true')) {
    ...
    Meteor.setInterval(function () {
      ...
      pendingNotifications.forEach(function (notification) {
        if (Push.debug) {
          console.log('Push (legacy): Found notification, reserving...');
        }
        //
        // if (process.env.NODE_ENV === 'development') {
        //   console.log('Push (legacy): DEV environment - not sending the notification', notification);
        //   return;
        // }
        ...
        if (reserved) {
          if (Push.debug) {
            console.log('Push (legacy): Notification reserved, processing...');
          }
          ...
          if (notification.query && notification.query === '' + notification.query) {
            try {
              ...
              throw new Error('Push (legacy): Error while parsing query string, Error: ' + err.message);
            }
          }
          ...
          try {
            ...
            console.log('Push (legacy): Exception sending notification');
    ...
    if (Push.debug) {
      console.log('Push (legacy): Send server is disabled');
    }
  }
};
