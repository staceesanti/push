...
Meteor.methods({
  'raix:configure-push-connections-legacy': (vetId) => {
    check(vetId, String);
    let vet = GlobalGenericApps.findOne({_id: vetId, $or: [{disabled: false}, {disabled: {$exists: false}}]});
    if (!vet) {
      vet = GlobalVets.findOne({_id: vetId, $or: [{disabled: false}, {disabled: {$exists: false}}]});
    }
    if (vet) {
      ...
      if (vet.firebaseConfig && vet.firebaseConfig.cloudMessaging) {
        Push.legacy.Connections[key].gcm = {
          apiKey: vet.firebaseConfig.cloudMessaging.serverKey,
          projectNumber: vet.firebaseConfig.cloudMessaging.projectNumber
        };
      }
