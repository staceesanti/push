...
let originals = {
  ...
  GlobalGenericApp: global.GlobalGenericApps,
  GlobalAppUtils: global.GlobalAppUtils,
  nodeEnvs: {
...
if (!global.GlobalGenericApps) {
  global.GlobalGenericApps = {};
}
...
describe('Testing push API', function () {
  ...
  firebaseAdminMocks.messagingMock = {
    send: function (...args) {
      ...
      if (args[0] && (args[0].topic === 'TOPIC_TO_FAIL_0' || args[0].topic === 'TOPIC_TO_FAIL_1')) {
        return new Promise((resolve, reject) => {
          reject(FirebaseMessagingError.fromServerError('RESOURCE_EXHAUSTED', 'SERVER_ERROR_MESSAGE', {raw: 'RAW ERROR'}));
        });
      }
  ...
  after(function () {
    ...
    global.GlobalGenericApps = originals.GlobalGenericApps;
    global.GlobalGenericApps = originals.GlobalAppUtils;
  ...
  it('Sends to tokens partially, and re-sends failed ones', function (done) {
    ...
    Meteor.setTimeout(function () {
      ...
      try {
        ...
        Push.notifications.find().fetch().forEach(raixNotification => {
          ...
          assert.equal(raixNotification.failedCount, 1);
          assert.ok(raixNotification.sent === false || raixNotification.sent === undefined);
          assert.ok(raixNotification.sending === false || raixNotification.sending === undefined);
        ...
        Push.appTokens.remove({_id: IOS_TOKEN_TO_FAIL_0._id});
        Push.appTokens.remove({_id: ANDROID_TOKEN_TO_FAIL_0._id});
      }
    }, 200);
  });

  it('Sends to topics partially, and re-sends failed ones', function (done) {
    ...
    Push.send({
      ...
      topics: ['TOPIC_0', 'TOPIC_TO_FAIL_0', 'TOPIC_1', 'TOPIC_TO_FAIL_1'],
      useFirebaseForMessaging: true
    });
    ...
    Meteor.setTimeout(function () {
      ...
      try {
        ...
        let failedTopics = [];
        Push.notifications.find().fetch().forEach(raixNotification => {
          failedTopics.push(...raixNotification.topics);
          ...
          verifyPushNotification(raixNotification, {
            ...
            expectedToken: undefined,
            expectedTokens: undefined,
            expectedTopics: raixNotification.topics
          });
        ...
        assert.equal(failedTopics.length, 2);

        assert.ok(failedTopics.includes('TOPIC_TO_FAIL_0'));
        assert.ok(failedTopics.includes('TOPIC_TO_FAIL_1'));
  ...
  it('Sends to tokens fails, re-sends notifications 5 times, than deletes them', function (done) {
    Push.appTokens.insert(IOS_TOKEN_TO_FAIL_0);
    Push.appTokens.insert(ANDROID_TOKEN_TO_FAIL_0);
    ...
    Push.send({
      ...
      tokens: [IOS_TOKEN_TO_FAIL_0.fcmToken, ANDROID_TOKEN_TO_FAIL_0.fcmToken],
      useFirebaseForMessaging: true
    });

    waitAndCheckFailedOnesSentToTokens(firebaseAdminSendSpy, pushEmitSpy, [1, 2, 3, 4, 5], 0, 10, done);

    // eslint-disable-next-line no-shadow
    function waitAndCheckFailedOnesSentToTokens(firebaseAdminSendSpy, pushEmitSpy, expectedFailedCounts, initialFirebaseAdminCallCount, expectedFirebaseAdminTotalCallCount, done) {
      Meteor.setTimeout(function () {

        try {
          if (expectedFailedCounts.length === 0) {
            try {
              assert.equal(Push.notifications.find().count(), 0);
              assert.equal(firebaseAdminSendSpy.callCount, 12);
          ...
          assert.equal(firebaseAdminSendSpy.callCount, initialFirebaseAdminCallCount + 2);
          ...
          Push.notifications.find().fetch().forEach(raixNotification => {
            ...
            assert.equal(raixNotification.failedCount, expectedFailedCounts[0]);
            assert.ok(raixNotification.sent === false || raixNotification.sent === undefined);
            assert.ok(raixNotification.sending === false || raixNotification.sending === undefined);
          ...
          Push.notifications.update({}, {$unset: {delayUntil: ''}}, {multi: true});
          waitAndCheckFailedOnesSentToTokens(firebaseAdminSendSpy, pushEmitSpy, expectedFailedCounts.slice(1), initialFirebaseAdminCallCount + 2, expectedFirebaseAdminTotalCallCount, done);
        } catch (e) {
          done(e);
        }
      }, 200);
  ...
  it('Sends to topics fails, re-sends notifications 5 times, than deletes them', function (done) {
    Push.appTokens.insert(IOS_TOKEN_TO_FAIL_0);
    Push.appTokens.insert(ANDROID_TOKEN_TO_FAIL_0);
    ...
    Push.send({
      ...
      topics: ['TOPIC_TO_FAIL_0', 'TOPIC_TO_FAIL_1'],
      useFirebaseForMessaging: true
    });

    waitAndCheckFailedOnesSentToTopics(firebaseAdminSendSpy, pushEmitSpy, [1, 2, 3, 4, 5], 0, 10, done);

    // eslint-disable-next-line no-shadow
    function waitAndCheckFailedOnesSentToTopics(firebaseAdminSendSpy, pushEmitSpy, expectedFailedCounts, initialFirebaseAdminCallCount, expectedFirebaseAdminTotalCallCount, done) {
      Meteor.setTimeout(function () {
        ...
        try {
          ...
          Push.notifications.update({}, {$unset: {delayUntil: ''}}, {multi: true});
          waitAndCheckFailedOnesSentToTopics(firebaseAdminSendSpy, pushEmitSpy, expectedFailedCounts.slice(1), initialFirebaseAdminCallCount + 2, expectedFirebaseAdminTotalCallCount, done);
        } catch (e) {
          done(e);
        }
      }, 200);
