...
let _validateDocument = function (notification) {
  ...
  check(notification, {
    from: String,
    title: Match.OneOf(String, null, undefined),
    text: Match.OneOf(String, null, undefined),
    ...
    topicsPlatform: Match.Optional(Object),
    ...
    useFirebaseForMessaging: Match.Optional(Boolean),
    failedCount: Match.Optional(Match.Integer),
    isDesktop: Match.Optional(Boolean),
    desktopAppId: Match.Optional(String),
    priority: Match.Optional(Match.Integer)
  });

  check(notification.title || notification.text, String);
...
Push.send = function (options) {
  ...
  }, _.pick(options, 'from', 'title', 'text', 'payload', 'badge', 'sound', 'notId', 'delayUntil', 'topics', 'topicsPlatform', 'useFirebaseForMessaging', 'failedCount', 'isDesktop', 'desktopAppId', 'priority'));
